...
namespace FreeCourse.Services.Catalog.Services
{
        ...
        : ICourseService
    {
        ...
        {
            var courses = await _courseCollection.Find(x => true)
                .ToListAsync();
        ...
        public async Task<Shared.Dtos.Response<CourseDto>> GetByCourseIdAsync(string id)
        {
            var course = await _courseCollection.Find(x => x.Id == id)
                .FirstOrDefaultAsync();
            if (course == null)
            {
        ...
        {
            var courses = await _courseCollection.Find(x => x.UserId == userId)
                .ToListAsync();
            ...
            if (courses.Any())
            {
                foreach (var course in courses)
                {
                    course.Category = await _categoryCollection
                        .Find(x => x.Id == course.CategoryId).FirstOrDefaultAsync();
                }
            }
            else
            {
